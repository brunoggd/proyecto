...
class Tdl(QMainWindow):
    ...
    def modificar_tarea(self, id_tarea, nombre, descripcion, categoria, prioridad, fecha):
        ...
        try:
            consulta = """UPDATE tareas
                        ...
                        WHERE id_tarea = %s"""
            campos = (nombre, descripcion, categoria, prioridad, fecha, id_tarea)
            cursor = self.conexion.cursor()
            cursor.execute(consulta,campos)
            self.conexion.commit()
            cursor.close()
    ...
    def modificar_tarea_seleccionada_modo_calendario(self):
        try:
            ...
            nueva_prioridad = ''
            fecha_adaptada_a_string = nueva_fecha_en_qdate.toString("yyyy-MM-dd")
            ...
            modificacion = self.modificar_tarea(id_tarea,
                                                nuevo_nombre_tarea,
            ...
            if not modificacion:
                QMessageBox.warning(self,"Error",f"No se pudo actualizar la tarea, error: {modificacion}")
                return
