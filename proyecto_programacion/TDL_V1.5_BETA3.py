...
class Tdl(QMainWindow):
    ...
    def conectar_mysql(self):
        try:
            ...
            cursor.execute("CREATE DATABASE IF NOT EXISTS base_de_datos_proyecto")
            cursor.execute("USE base_de_datos_proyecto")
            cursor.close()
    ...
    def cargar_tareas(self):
        if not self.conexion:
            ...
            return
        try:
            consulta = "SELECT id_tarea,nombre,descripcion,categoria,prioridad,fecha FROM tareas"
            cursor = self.conexion.cursor()
            cursor.execute(consulta)
            ...
            for fila in filas:
                ...
                fecha_en_qdate = QDate.fromString(str(fecha),"yyyy-MM-dd")
                ...
                self.diccionario_tareas_modo_calendario.setdefault(fecha_en_qdate,[]).append(tareas_dict)
    ...
    def modificar_tarea(self, id_tarea, nombre, descripcion, categoria, prioridad, fecha):
        ...
        try:
            consulta = """UPDATE tareas
                        ...
                        WHERE id_tarea = %s"""
            campos = (id_tarea, nombre, descripcion, categoria, prioridad, fecha)
            cursor = self.conexion.cursor()
            cursor.execute(consulta,campos)
            self.conexion.commit()
            cursor.close()
        ...
        if nombre_tarea and descripcion_tarea:
            ...
            if respuesta == QMessageBox.Yes:
                if self.buscar_tareas_duplicadas(nombre_tarea,descripcion_tarea):
                    ...
                    item.setData(Qt.UserRole + 3, categoria_elegida,)
    ...
    def modificar_tarea_seleccionada_modo_calendario(self):
        try:
            ...
            nombre_tarea_original = tarea_seleccionada.data(Qt.UserRole + 1)
            descripcion_tarea_original = tarea_seleccionada.data(Qt.UserRole + 2)
            fecha_original_en_qdate = tarea_seleccionada.data(Qt.UserRole + 5)
            ...
            tarea_seleccionada.setData(Qt.UserRole + 1, nuevo_nombre_tarea)
            tarea_seleccionada.setData(Qt.UserRole + 2,nueva_descripcion_tarea)
            tarea_seleccionada.setData(Qt.UserRole + 3, nueva_categoria)
            tarea_seleccionada.setData(Qt.UserRole + 4, nueva_prioridad)
            tarea_seleccionada.setData(Qt.UserRole + 5, nueva_fecha_en_qdate)
...
"""
    ...
    def opciones(self):
        ...
        self.slider_volumen.valueChanged.connect(self.guardar_config_mysql)
