...
class Tdl(QMainWindow):
    ...
    def conectar_mysql(self):
        try:
            ...
            QMessageBox.warning(self,"Error",rdo)
            return None
        
    def crear_base_de_datos(self):
        try:
            consulta = f"CREATE DATABASE IF NOT EXISTS base_de_datos_proyecto"
            ...
            rdo=f"Error al crear la base de datos: {e}"
            QMessageBox.warning(self,"Error",rdo)
    ...
    def crear_tabla_config(self):
        try:
            consulta = f"""CREATE TABLE IF NOT EXISTS config(
                            ...
                            tema_elegido VARCHAR(6),
                            pantalla_completa BOOLEAN,
                            volumen INT)"""
    ...
    def guardar_config(self):
        try:
            configuracion = (self.tema_elegido,self.pantalla_completa,self.volumen)

            consulta = f"""INSERT INTO config values(tema_elegido,pantalla_completa,volumen)
                        VALUES (%s,%s,%s)"""
            
            cursor = self.conexion.cursor()
            cursor.execute(consulta,configuracion)

            self.conexion.commit()
            cursor.close()

        except Error as e:
            rdo=f"Error al guardar la configuración: {e}"
            QMessageBox.warning(self,"Error",rdo)
    ...
    def actualizar_config(self):
        try:
            ...
            config_actualizada = (self.tema_elegido,int(self.pantalla_completa),self.volumen)
